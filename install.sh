...
check_token() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "${BLUE}[+]               LICENSY FAZXZ OFFC                [+]${NC}"
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
  echo -e "                                                       "
  echo -e "${YELLOW}MASUKAN AKSES TOKEN :${NC}"
  read -r USER_TOKEN

  if [ "$USER_TOKEN" = "FazxzofficialBotz" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Buy dulu Gih Ke Fazxz${NC}"
    echo -e "${YELLOW}TELEGRAM : @Fazxzofficial${NC}"
    echo -e "${YELLOW}WHATSAPP : 6283897503330${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©Fazxz${NC}"
    exit 1
  fi
  clear
}
...
while true; do
  ...
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : Fazxz offc   ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Fazxz offc   ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Revxz        ${NC}"
